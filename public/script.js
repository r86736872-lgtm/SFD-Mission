...
// ==========================================
// 1B. STATE GLOBAL: CHAT & VIDEO CALL (CANVASSING)
// ==========================================
window.chatRoomsOpened = window.chatRoomsOpened || new Set(); // room_id yang pernah dibuka di sesi ini, utk dibersihkan saat canvassing berhenti
let currentChatRoomId = null;
let currentChatPeerId = null;
let chatMessagesListenerRef = null; // ref Firebase yg sedang di-listen, utk di-off() saat ganti chat
let chatCallListenerRef = null; // ref sinyal WebRTC yg sedang di-listen

// Notifikasi chat masuk (global, tidak tergantung panel yg sedang dibuka)
window.globalChatInboxRefs = window.globalChatInboxRefs || {}; // { roomId: databaseRef } - listener child_added per room rekan aktif
window.peerDisplayNames = window.peerDisplayNames || {}; // { username: nama } - utk teks notifikasi

// [PERBAIKAN BUG]: Notifikasi panggilan masuk (global, tidak tergantung
// panel chat yg sedang dibuka). Sebelumnya listenIncomingCall() hanya
// dipasang saat user membuka panel chat rekan ybs (openChatPanel), jadi
// panggilan masuk tidak akan pernah terlihat kalau chat belum dibuka.
window.globalCallInboxRefs = window.globalCallInboxRefs || {}; // { roomId: databaseRef } - listener offer per room rekan aktif
// Room id panggilan yg sedang berlangsung/diterima, dilacak terpisah dari
// currentChatRoomId supaya tetap valid walau panggilan diterima tanpa
// membuka panel chat rekan ybs terlebih dahulu.
window.activeCallRoomId = window.activeCallRoomId || null;

let localMediaStream = null;
let remoteWebRTCPeerConnection = null;
let currentCallType = null; // 'audio' | 'video'
let iceCandidateListeners = []; // { ref, callback } utk di-off() saat call berakhir

// [PERBAIKAN BUG]: Notifikasi ke lawan bicara saat panggilan ditolak/diakhiri.
// Sebelumnya endWebRTCCall/rejectIncomingCall hanya menghapus data sinyal
// tanpa memberi tahu pihak lain secara eksplisit, sehingga UI panggilan di
// sisi lawan tidak pernah tertutup (hanya mengandalkan deteksi WebRTC
// onconnectionstatechange yang lambat/tidak selalu terpicu). Sekarang dipakai
// field canvassing_calls/{roomId}/status: "ringing" | "ended" | "rejected".
let callStatusListenerRef = null;
let callStatusListenerCallback = null;

// [PERBAIKAN BUG]: "Uncaught ReferenceError: ringtoneAudio is not defined"
// Sebelumnya `ringtoneAudio` dideklarasikan dengan `let` DI DALAM fungsi
// notifyIncomingChat() (padahal tidak pernah dipakai di situ), sementara
// listenIncomingCall(), acceptIncomingCall(), endWebRTCCall(), dan
// showIncomingCallDialog() memakainya seolah-olah variabel global.
// Kalau notifyIncomingChat() belum pernah jalan, variabel itu memang
// belum pernah ada -> ReferenceError. Sekarang dijadikan satu instance
// Audio global yang diinisialisasi sekali di awal.
const RINGTONE_URL =
  "https://www.soundjay.com/phone/sounds/phone-ringing-01.mp3";
let ringtoneAudio = new Audio(RINGTONE_URL);
...
function tampilkanPopupSesiBerakhir() {
  ...
  if (window.sessionWatcherInterval)
    clearInterval(window.sessionWatcherInterval);
...
function muatLebihBanyakDataUtama() {
  ...
  firestore
    ...
    .then((snapshot) => {
      ...
      mainDbLastVisibleDoc =
        snapshot.docs[snapshot.docs.length - 1] || mainDbLastVisibleDoc;
...
function getPTDateString() {
  let ptDate = new Date().toLocaleString("en-US", {
...
function evaluasiBatasKuota() {
  let sisaReads =
    ((FIRESTORE_QUOTA.MAX_READS - currentQuota.reads) /
      FIRESTORE_QUOTA.MAX_READS) *
    100;
  let sisaWrites =
    ((FIRESTORE_QUOTA.MAX_WRITES - currentQuota.writes) /
      FIRESTORE_QUOTA.MAX_WRITES) *
    100;
  let sisaDeletes =
    ((FIRESTORE_QUOTA.MAX_DELETES - currentQuota.deletes) /
      FIRESTORE_QUOTA.MAX_DELETES) *
    100;
  ...
  if (sisaReads <= 3) {
    ...
    if (loggedInUser && !quotaAlertsShown.r3) {
      showDialog(
        "Akses Ditutup",
        "Kuota Reads Firestore habis (Sisa <= 3%). Sistem dinonaktifkan.",
        "alert",
        () => logout(),
      );
    ...
    showToast(
      "Peringatan Kritis: Kuota Reads sisa <= 5%. Segera selesaikan pekerjaan!",
      "error",
    );
  ...
  if (sisaWrites <= 3) {
    ...
    if (loggedInUser && loggedInUser.role !== "admin" && !quotaAlertsShown.w3) {
      showDialog(
        "Akses Ditutup",
        "Kuota Writes habis. Akses user biasa dihentikan hingga besok.",
        "alert",
        () => logout(),
      );
...
function trackFirestoreUsage(type, count) {
  ...
  console.log(
    `[QuotaTracker] +${count} ${type} dicatat. Buffer saat ini:`,
    JSON.stringify(_quotaUsageBuffer),
  );
  ...
  if (!_quotaFlushInterval) {
    ...
    _quotaFlushInterval = setInterval(
...
function flushQuotaUsageBuffer() {
  ...
  console.log(
    "[QuotaTracker] flushQuotaUsageBuffer jalan, buffer:",
    JSON.stringify(_quotaUsageBuffer),
  );
      ...
      ref.transaction(
        ...
        (error, committed) => {
          if (error) {
            ...
            console.error(
              `[QuotaTracker] GAGAL kirim ${count} ${type} ke RTDB:`,
              error,
            );
            ...
            // [LOG SEMENTARA UNTUK DEBUG]
            console.log(
              `[QuotaTracker] BERHASIL kirim ${count} ${type} ke RTDB (committed=${committed})`,
            );
          }
        },
      );
    ...
    .then((snapshot) => {
      ...
      pengajuanRef.on("child_added", (childSnap) => {
        ...
        if (
          ...
          typeof renderAdminEformSubmissions === "function"
        ) {
          renderAdminEformSubmissions();
        }
      });
...
function updateSingleQuotaChart(canvasId, used, max, color, key) {
  ...
  if (subTextEl)
    subTextEl.innerText = `${used.toLocaleString("id-ID")} / ${max.toLocaleString("id-ID")}`;
...
async function handleLogin() {
  ...
  try {
    ...
    if (errCode === "auth/network-request-failed") {
      pesanError =
        "Koneksi internet terputus. Pastikan sinyal stabil lalu coba lagi.";
      showToast(pesanError, "error");
      resetLoginSistem();
      return;
    } else if (errCode === "auth/too-many-requests") {
      pesanError =
        "Akses ditahan sementara oleh server karena aktivitas mencurigakan.";
      showToast(pesanError, "error");
      resetLoginSistem();
      return;
    ...
    if (!USERS[user]) {
      ...
      if (currentAttempts >= 5) {
        ...
        pesanError =
          "Akses Diblokir! 5x password salah. Akun terkunci selama 5 menit.";
      } else {
...
loggedInUser = {
  ...
  name: USERS[user]?.name || user.toUpperCase(), // Tambahkan baris ini untuk menyimpan nama asli
};
...
function showLoginSuccessPopup(user) {
  ...
  const popupFallbackUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=0D8ABC&color=fff`;
  const isPopupPhotoValid =
    typeof currentUserData.photo === "string" &&
    currentUserData.photo.trim() !== "" &&
    currentUserData.photo !== "null" &&
    currentUserData.photo !== "undefined";
  let photoUrl = isPopupPhotoValid ? currentUserData.photo : popupFallbackUrl;

  const popupPhotoEl = document.getElementById("popup-user-photo");
  popupPhotoEl.onerror = function () {
    this.onerror = null;
    this.src = popupFallbackUrl;
  };
  popupPhotoEl.src = photoUrl;
...
function initApp() {
  ...
  if (headerPhoto) {
    // Sesuaikan "nama" dengan nama field yang ada di database/objek Anda (bisa "nama", "name", atau "displayName")
    const namaUser = currentUserData.nama || currentUserData.name || "User";

    // Pastikan format URL aman meskipun nama mengandung spasi
    const fallbackPhotoUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(namaUser)}&background=0D8ABC&color=fff`;

    // [PERBAIKAN BUG]: foto profil tidak terload / tampil ikon rusak.
    // Penyebab:
    // 1. Nilai currentUserData.photo bisa berupa string kosong/"null"/"undefined"
    //    (bukan hanya undefined), yang lolos dari fallback "||" versi lama.
    // 2. Jika URL foto (mis. dari imgbb) sudah tidak valid/terhapus/gagal
    //    dimuat, browser menampilkan ikon rusak + teks alt secara permanen
    //    karena tidak ada penanganan onerror -> kini otomatis fallback ke
    //    ui-avatars, dan onerror di-nonaktifkan setelahnya agar tidak
    //    terjadi infinite-loop jika fallback pun gagal dimuat.
    const isPhotoValid =
      typeof currentUserData.photo === "string" &&
      currentUserData.photo.trim() !== "" &&
      currentUserData.photo !== "null" &&
    ...
    headerPhoto.onerror = function () {
      this.onerror = null;
      this.src = fallbackPhotoUrl;
    };
    headerPhoto.src = isPhotoValid ? currentUserData.photo : fallbackPhotoUrl;
  }
        ...
        let parsedData = jsonRows
          .map((row) => {
            ...
            let findCol = (exactKeywords, partialKeyword, fallbackIndex) => {
              ...
              return (
                partial || (fallbackIndex !== null ? k[fallbackIndex] : null)
              );
            };
...
const lockOverlay = document.getElementById("canvassing-lock-overlay");
 ...
 if (lockOverlay) {
  ...
  // Bersihkan seluruh histori chat & sisa sinyal WebRTC begitu canvassing dimatikan,
  // supaya RTDB tidak menumpuk data sementara.
  if (typeof endWebRTCCall === "function") endWebRTCCall(false);
  if (window.chatRoomsOpened && window.chatRoomsOpened.size > 0) {
    window.chatRoomsOpened.forEach((roomId) => {
      database.ref(`canvassing_chats/${roomId}`).remove();
      database.ref(`canvassing_calls/${roomId}`).remove();
    });
    window.chatRoomsOpened.clear();
  }
  if (typeof closeChatPanel === "function") closeChatPanel();
  if (typeof detachAllChatInboxListeners === "function")
    detachAllChatInboxListeners();
...
function toggleCanvassing() {
  ...
  if (overlay) {
    if (!overlay.classList.contains("hidden")) {
      // PROSES UNLOCK (MENUTUP OVERLAY)
      overlay.classList.add("opacity-0");
      setTimeout(() => {
        overlay.classList.add("hidden");
        overlay.classList.remove("flex");
        overlay.innerHTML = ""; // Bersihkan konten DOM
      }, 300);
      ...
      // PROSES LOCK (MENAMPILKAN OVERLAY)
      // 1. Render konten UI secara dinamis
      overlay.innerHTML = `
        <div class="flex flex-col items-center justify-center w-full h-full bg-slate-900/40 text-center space-y-4 p-6">
            <div class="bg-white/20 p-5 rounded-full backdrop-blur-md border border-white/30 shadow-lg">
                <svg class="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
                </svg>
            </div>
            <h2 class="text-2xl font-bold text-white drop-shadow-md">Akses Data Terkunci</h2>
            <p class="text-sm text-white/90 drop-shadow-sm max-w-sm">
                Layar utama ditutup demi keamanan dan privasi data konsumen selama Mode Canvassing aktif.
            </p>
            <button onclick="toggleCanvassing()" class="mt-4 bg-sky-600 hover:bg-sky-700 text-white font-bold py-3 px-8 rounded-full shadow-lg transition-all transform hover:scale-105 border border-sky-400 cursor-pointer">
                Buka Kunci Layar
            </button>
        </div>
      `;

      // 2. Tampilkan elemen via classList (hapus inline style yang konflik)
      overlay.classList.remove("hidden");
      overlay.classList.add("flex");
      overlay.style.display = "";

      // 3. Trigger transisi opacity
      setTimeout(() => {
        overlay.classList.remove("opacity-0");
      }, 10);
  ...
  showDialog(
    ...
    () => {
      tempAdminMapMarkers.forEach((item) => {
        database.ref("ocm_task_markers").push({
          lat: item.lat,
          lng: item.lng,
  ...
  database.ref("ocm_canvassing").on("value", (snapshot) => {
    ...
    for (let id in window.userPeerMarkers) {
      if (
        ...
        detachChatInboxListener(id); // rekan offline -> lepas listener notifikasi chat-nya
      }
    }
      ...
      Object.keys(data).forEach((id) => {
        ...
        const popupContentHtml = `
            <div class="text-center p-1 min-w-[140px]">
                ...
                ${user.sos ? '<p class="text-[10px] font-bold text-white bg-rose-500 px-2 py-1 rounded shadow-sm animate-pulse mb-2">🚨 Darurat SOS!!!</p>' : '<p class="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded mb-2">Aktif Canvassing</p>'}
                
                <!-- TOMBOL CHAT BARU -->
                <button onclick="openChatPanel('${id}', '${user.nama || id}')" class="bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-bold px-3 py-1.5 rounded shadow cursor-pointer transition w-full flex items-center justify-center gap-1">
                    💬 Chat Rekan
                </button>
            </div>`;
        ...
        // Pastikan listener notifikasi chat aktif utk rekan ini (idempotent)
        ensureChatInboxListener(id, user.nama || id);
      });
    }
  });
      ...
      Object.keys(data).forEach((id) => {
        ...
        if (markerData.assignedTo === loggedInUser.username) {
          ...
          marker.on("popupopen", () => {
            ...
            const btnRuteLokal = document.getElementById(
              `btn-rute-lokal-${id}`,
            );
...
async function openCameraPanel(taskId) {
  ...
  if (!guideOverlay) {
    const videoContainer =
      document.getElementById("camera-stream").parentElement;
...
async function bukaKameraStream() {
  try {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      showToast(
        "Akses kamera diblokir browser. Pastikan web menggunakan HTTPS!",
        "error",
      );
      closeCameraPanel();
      return;
...
// ==========================================
// 9. PANEL CHAT MENGAMBANG & VIDEO CALL P2P (WEBRTC)
// ==========================================

// Room id deterministik: kombinasi 2 uid, diurutkan alfabet supaya kedua
// pihak selalu menghasilkan room_id yang sama persis.
function getCanvassingRoomId(uidA, uidB) {
  return [uidA, uidB].sort().join("_");
}

// --- A. BUKA / TUTUP PANEL ---

window.openChatPanel = function (userIdRekan, namaRekan) {
  if (!loggedInUser || !loggedInUser.username) return;
  if (!userIdRekan) return;

  // Jika sedang membuka chat lain, lepas listener chat & sinyal call sebelumnya
  detachChatListeners();

  currentChatPeerId = userIdRekan;
  currentChatRoomId = getCanvassingRoomId(loggedInUser.username, userIdRekan);
  window.chatRoomsOpened.add(currentChatRoomId);

  const nameEl = document.getElementById("chat-target-name");
  const initialEl = document.getElementById("chat-target-initial");
  if (nameEl) nameEl.innerText = namaRekan || userIdRekan;
  if (initialEl)
    initialEl.innerText = (namaRekan || userIdRekan).charAt(0).toUpperCase();
  ...
  listenChatMessages(currentChatRoomId);
  listenIncomingCall(currentChatRoomId);
};
...
window.closeChatPanel = function () {
  ...
  // Jika sedang menelepon, hentikan dulu panggilannya
  if (remoteWebRTCPeerConnection) endWebRTCCall(true);
  detachChatListeners();
...
function detachChatListeners() {
  if (chatMessagesListenerRef) {
    chatMessagesListenerRef.off();
    chatMessagesListenerRef = null;
  }
  if (chatCallListenerRef) {
    chatCallListenerRef.off();
    chatCallListenerRef = null;
  }
}

// Tambahkan variabel global untuk mencatat ID pesan yang sudah dinotifikasi
window.notifiedMessageIds = window.notifiedMessageIds || new Set();

function ensureChatInboxListener(peerUsername, peerName) {
  if (!loggedInUser || !loggedInUser.username) return;
  if (!peerUsername || peerUsername === loggedInUser.username) return;

  window.peerDisplayNames[peerUsername] = peerName || peerUsername;

  const roomId = getCanvassingRoomId(loggedInUser.username, peerUsername);
  if (window.globalChatInboxRefs[roomId]) return; // sudah terpasang

  // PERBAIKAN BUG #1: Beri toleransi waktu mundur 2 menit (120000 ms)
  // untuk menutupi perbedaan antara jam lokal HP dengan jam server Firebase.
  const waktuToleransi = Date.now() - 120000;

  const ref = database
    .ref(`canvassing_chats/${roomId}`)
    .orderByChild("timestamp")
    .startAt(waktuToleransi);

  ref.on("child_added", (snapshot) => {
    const msg = snapshot.val();
    const msgKey = snapshot.key; // ID unik pesan

    if (!msg || msg.from === loggedInUser.username) return; // abaikan pesan sendiri
    if (msg.to !== loggedInUser.username) return; // bukan utk kita

    // Kalau panel chat dgn pengirim ini sedang aktif dibuka, abaikan
    if (currentChatRoomId === roomId) return;

    // PERBAIKAN BUG #1: Cegah notifikasi ganda jika pesan sudah pernah diproses
    if (window.notifiedMessageIds.has(msgKey)) return;
    window.notifiedMessageIds.add(msgKey);

    // Jaga agar Set memori tidak bengkak (maksimal simpan 100 ID terakhir)
    if (window.notifiedMessageIds.size > 100) {
      const firstItem = window.notifiedMessageIds.values().next().value;
      window.notifiedMessageIds.delete(firstItem);
    }

    notifyIncomingChat(msg.from, roomId);
  });

  window.globalChatInboxRefs[roomId] = ref;
}
function detachChatInboxListener(peerUsername) {
  if (!loggedInUser || !loggedInUser.username || !peerUsername) return;
  const roomId = getCanvassingRoomId(loggedInUser.username, peerUsername);
  if (window.globalChatInboxRefs[roomId]) {
    window.globalChatInboxRefs[roomId].off();
    delete window.globalChatInboxRefs[roomId];
  }
}

function detachAllChatInboxListeners() {
  Object.keys(window.globalChatInboxRefs).forEach((roomId) => {
    window.globalChatInboxRefs[roomId].off();
    delete window.globalChatInboxRefs[roomId];
  });
}

function notifyIncomingChat(fromUsername, roomId) {
  const namaRekan = window.peerDisplayNames[fromUsername] || fromUsername;

  // === AUDIO NOTIFICATIONS ===
  // Ganti URL berikut dengan URL audio Anda (bisa file lokal atau online)
  const NOTIFICATION_SOUND_URL =
    "https://www.soundjay.com/misc/sounds/bell-ringing-05.mp3";

  // Inisialisasi audio elements
  // (ringtoneAudio memakai instance global yang sudah dideklarasikan di
  // bagian atas file - lihat komentar [PERBAIKAN BUG] di dekat RTC_CONFIG)
  let notificationAudio = new Audio(NOTIFICATION_SOUND_URL);

  // Putar bunyi notifikasi
  notificationAudio.currentTime = 0;
  notificationAudio
    .play()
    .catch((e) => console.warn("Gagal memutar notifikasi:", e));

  // PERBAIKAN BUG #4: Buat elemen Toast secara mandiri agar aman dan bebas race condition
  let container = document.getElementById("toast-container");
  if (!container) {
    container = document.createElement("div");
    container.id = "toast-container";
  ...
  // Wajib menambahkan 'pointer-events-auto' agar bisa diklik, karena containernya pointer-events-none
  toast.className = `flex items-center gap-3 px-4 py-3.5 rounded-2xl shadow-xl border border-blue-200 transform translate-x-full opacity-0 transition-all duration-400 ease-out z-[9999] min-w-[280px] max-w-sm cursor-pointer hover:scale-[1.02] bg-blue-600 text-white pointer-events-auto`;
  ...
  toast.innerHTML = `
    ...
    <div class="flex-1 flex flex-col">
        <span class="text-[10px] font-bold uppercase text-blue-500 tracking-wider">Pesan Baru</span>
        <span class="text-sm font-semibold tracking-wide">${namaRekan}</span>
    </div>
    <button class="close-btn text-blue-400 hover:text-blue-700 hover:bg-blue-100 rounded-full transition-colors p-1.5">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
    </button>
  `;

  // Listener dikaitkan langsung ke toast ini saja, sangat aman.
  toast.addEventListener("click", (e) => {
    // Jika yang diklik adalah tombol silang (Tutup)
    if (e.target.closest(".close-btn")) {
      toast.remove();
      return;
    }
    // Buka panel chat dan hapus toast
    window.openChatPanel(fromUsername, namaRekan);
    toast.remove();
  });
  ...
  // Animasi keluar setelah 4 detik (jika belum diklik)
  setTimeout(() => {
...
// --- B. CHAT REALTIME (FIREBASE RTDB, SEMENTARA) ---

function listenChatMessages(roomId) {
  chatMessagesListenerRef = database.ref(`canvassing_chats/${roomId}`);
  chatMessagesListenerRef.on("value", (snapshot) => {
    ...
    Object.keys(data)
      ...
      .forEach((key) => {
        const msg = data[key];
        const isMine = msg.from === loggedInUser.username;
        const bubble = document.createElement("div");
        bubble.className = `max-w-[80%] px-3 py-2 rounded-2xl text-xs break-words ${
          isMine
            ? "bg-blue-600 text-white self-end rounded-br-sm"
            : "bg-white text-slate-700 border border-slate-200 self-start rounded-bl-sm"
        }`;
...
window.sendChatMessage = function () {
  ...
  if (!currentChatRoomId || !loggedInUser || !loggedInUser.username) {
    if (typeof showToast === "function")
      showToast("Buka chat rekan kerja dahulu.", "error");
    return;
  }

  const text = input.value.trim();
  const roomIdSaatKirim = currentChatRoomId; // jaga-jaga kalau panel diganti sebelum promise selesai
  input.value = "";
  input.disabled = true;

  database
    .ref(`canvassing_chats/${roomIdSaatKirim}`)
    .push({
    ...
    .catch((err) => {
      // Sebelumnya kegagalan di sini (mis. ditolak Firebase Rules) diam-diam
      // saja -> pesan hilang tanpa keterangan & tidak pernah muncul di panel.
      console.error("Gagal mengirim pesan chat:", err);
      if (typeof showToast === "function")
        showToast(`Pesan gagal terkirim: ${err.message || err}`, "error");
      // Kembalikan teks ke kolom input supaya tidak hilang begitu saja
      if (currentChatRoomId === roomIdSaatKirim) input.value = text;
    })
    .finally(() => {
...
if (!window._chatEnterKeyBound) {
  ...
  window._chatEnterKeyBound = true;
}

// --- C. TELEPON & VIDEO CALL P2P (WEBRTC, SIGNALING VIA RTDB) ---

window.currentIncomingOffer = null;
window.currentIncomingRoomId = null;
...
// [FUNGSI BARU]: Mengganti mode tombol di bawah (Terima/Tolak vs Mute/Speaker/Tutup)
function setCallUIMode(mode) {
  ...
  if (mode === "incoming") {
    ...
    if (incomingPanel) incomingPanel.classList.remove("hidden");
  } else {
    // Mode 'active' (Tersambung / Memanggil)
    if (activePanel) activePanel.classList.remove("hidden");
    if (incomingPanel) incomingPanel.classList.add("hidden");
  }
}

function setupCallUI(type, peerId) {
  const remoteVideo = document.getElementById("remote-video");
  const localVideo = document.getElementById("local-video");
  const audioProfile = document.getElementById("audio-call-profile");
  const profileName = document.getElementById("call-profile-name");
  const profileImg = document.getElementById("call-profile-img");

  const displayName =
    window.peerDisplayNames[peerId] ||
    (USERS && USERS[peerId] ? USERS[peerId].name : peerId);
  if (profileName) profileName.innerText = displayName;

  if (profileImg) {
    const userPhoto = USERS && USERS[peerId] ? USERS[peerId].photo : null;
    const isPhotoValid =
      typeof userPhoto === "string" &&
      userPhoto.trim() !== "" &&
      userPhoto !== "null" &&
      userPhoto !== "undefined";
    profileImg.src = isPhotoValid
      ? userPhoto
      : `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=214360&color=fff`;
  }

  // Jika Video Call, tampilkan elemen video
  if (type === "video") {
    if (remoteVideo) remoteVideo.classList.remove("hidden");
    if (localVideo) localVideo.classList.remove("hidden");
    // Opacity dikurangi agar tidak menutupi video secara total jika diperlukan
    if (audioProfile) audioProfile.classList.add("opacity-0");
  } else {
    if (remoteVideo) remoteVideo.classList.add("hidden");
    if (localVideo) localVideo.classList.add("hidden");
    if (audioProfile) {
      audioProfile.classList.remove("hidden");
      audioProfile.classList.remove("opacity-0");
    }
  }
}
...
function createPeerConnection(roomId) {
  ...
  pc.onicecandidate = (event) => {
    if (event.candidate) {
      database
        .ref(`canvassing_calls/${roomId}/candidates/${loggedInUser.username}`)
  ...
  pc.onconnectionstatechange = () => {
    if (pc.connectionState === "connected") {
      setCallStatus("00:00"); // Bisa diganti timer asli nanti
    } else if (
      pc.connectionState === "disconnected" ||
      pc.connectionState === "failed"
    ) {
      setCallStatus("Panggilan terputus");
      window.endWebRTCCall(true);
    }
  };
  return pc;
}

function listenForRemoteIceCandidates(roomId, remoteUid) {
  const ref = database.ref(
    `canvassing_calls/${roomId}/candidates/${remoteUid}`,
  );
  const callback = (snapshot) => {
    const candidate = snapshot.val();
    if (candidate && remoteWebRTCPeerConnection) {
      remoteWebRTCPeerConnection
        .addIceCandidate(new RTCIceCandidate(candidate))
        .catch((err) => console.error("Gagal menambahkan ICE candidate:", err));
    }
  };
  ref.on("child_added", callback);
  iceCandidateListeners.push({ ref, callback });
}

function listenForCallStatus(roomId) {
  stopListeningForCallStatus();
  callStatusListenerRef = database.ref(`canvassing_calls/${roomId}/status`);
  callStatusListenerCallback = (snapshot) => {
    const status = snapshot.val();
    if (status === "ended") {
      if (window._ringtoneDialogClose) window._ringtoneDialogClose();
      window.currentIncomingOffer = null;
      if (remoteWebRTCPeerConnection || localMediaStream) {
        if (typeof showToast === "function")
          showToast("Panggilan diakhiri.", "info");
        window.endWebRTCCall(false);
      } else {
        if (typeof showToast === "function")
          showToast("Panggilan dibatalkan.", "info");
  ...
  callStatusListenerRef.on("value", callStatusListenerCallback);
}

function stopListeningForCallStatus() {
  if (callStatusListenerRef && callStatusListenerCallback) {
    callStatusListenerRef.off("value", callStatusListenerCallback);
  }
  callStatusListenerRef = null;
  callStatusListenerCallback = null;
}

// ---------------------------------------------
// LOGIKA MEMULAI & MENERIMA PANGGILAN
// ---------------------------------------------

window.startWebRTCCall = function (type) {
  if (!currentChatRoomId || !currentChatPeerId) {
    if (typeof showToast === "function")
      showToast("Buka chat rekan kerja dahulu.", "error");
    return;
  }
  if (remoteWebRTCPeerConnection) return;

  currentCallType = type;
  const roomId = currentChatRoomId;

  // Set UI jadi Mode Aktif menelpon (Foto 1)
  setCallUIMode("active");
  setupCallUI(type, currentChatPeerId);
  showVideoCallContainer();
  setCallStatus("Memanggil...");

  if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
    if (typeof showToast === "function")
      showToast("Fitur panggilan diblokir browser.", "error");
    hideVideoCallContainer();
    return;
  }
  ...
  navigator.mediaDevices
    ...
    .then((stream) => {
      ...
      remoteWebRTCPeerConnection = createPeerConnection(roomId);
      stream
        .getTracks()
        .forEach((track) => remoteWebRTCPeerConnection.addTrack(track, stream));

      listenForRemoteIceCandidates(roomId, currentChatPeerId);
      return remoteWebRTCPeerConnection.createOffer();
    })
    .then((offer) => remoteWebRTCPeerConnection.setLocalDescription(offer))
    .then(() => {
      return database.ref(`canvassing_calls/${roomId}/offer`).set({
        sdp: remoteWebRTCPeerConnection.localDescription.sdp,
        type: remoteWebRTCPeerConnection.localDescription.type,
    ...
    .then(() => {
      listenForCallStatus(roomId);
      return database.ref(`canvassing_calls/${roomId}/status`).set("ringing");
    })
    .then(() => {
      const answerRef = database.ref(`canvassing_calls/${roomId}/answer`);
      const answerCallback = (snapshot) => {
        const answer = snapshot.val();
        if (
          answer &&
          remoteWebRTCPeerConnection &&
          !remoteWebRTCPeerConnection.currentRemoteDescription
        ) {
          remoteWebRTCPeerConnection.setRemoteDescription(
            new RTCSessionDescription(answer),
          );
        }
      };
      answerRef.on("value", answerCallback);
      iceCandidateListeners.push({ ref: answerRef, callback: answerCallback });
    })
    .catch((err) => {
      console.error("Gagal memulai panggilan:", err);
      if (typeof showToast === "function")
        showToast("Gagal memulai sistem panggilan.", "error");
      window.endWebRTCCall(true);
    });
};

function listenIncomingCall(roomId) {
  chatCallListenerRef = database.ref(`canvassing_calls/${roomId}/offer`);
  chatCallListenerRef.on("value", (snapshot) => {
    const offer = snapshot.val();
    if (!offer || offer.caller === loggedInUser.username) return;
    if (remoteWebRTCPeerConnection) return;
    ...
    ringtoneAudio
      .play()
      .catch((e) => console.warn("Gagal memutar nada dering:", e));

    // Panggil UI iPhone untuk Incoming Call
    showIncomingCallPhoneUI(roomId, offer);
  });
}

// [FUNGSI BARU]: Memunculkan UI Layar Penuh Layaknya Telepon Masuk
function showIncomingCallPhoneUI(roomId, offer) {
  window.currentIncomingOffer = offer;
  window.currentIncomingRoomId = roomId;

  const callerId = offer.caller || currentChatPeerId || roomId;

  // Setel foto, nama & mode (Mode Foto 2)
  setupCallUI(offer.callType || "audio", callerId);
  setCallStatus("Panggilan Masuk...");
  setCallUIMode("incoming");
  showVideoCallContainer();

  window._ringtoneDialogClose = () => {
    ringtoneAudio.pause();
    ringtoneAudio.currentTime = 0;
  };
}

// Handler Global untuk Tombol HTML
window.acceptPhoneCall = function () {
  if (window.currentIncomingRoomId) {
    window.acceptIncomingCall(window.currentIncomingRoomId);
  }
};

window.rejectPhoneCall = function () {
  if (window.currentIncomingRoomId) {
    window.rejectIncomingCall(window.currentIncomingRoomId);
  }
};

window.acceptIncomingCall = function (roomIdParam, offerParam) {
  const roomId = roomIdParam || window.currentIncomingRoomId;
  const offer = offerParam || window.currentIncomingOffer;
  if (!offer) return;
  ...
  currentCallType = offer.callType || "audio";
  listenForCallStatus(roomId);

  // Ubah mode ke Mode Aktif (Foto 1) setelah diangkat
  setCallUIMode("active");
  setupCallUI(currentCallType, offer.caller || currentChatPeerId);
  ...
  navigator.mediaDevices
    .getUserMedia({ audio: true, video: currentCallType === "video" })
    .then((stream) => {
      localMediaStream = stream;
      const localVideo = document.getElementById("local-video");
      if (localVideo) localVideo.srcObject = stream;
      ...
      listenForRemoteIceCandidates(roomId, offer.caller || currentChatPeerId);

      return remoteWebRTCPeerConnection.setRemoteDescription(
        new RTCSessionDescription({ sdp: offer.sdp, type: offer.type }),
      );
    })
    .then(() => remoteWebRTCPeerConnection.createAnswer())
    .then((answer) => remoteWebRTCPeerConnection.setLocalDescription(answer))
    .then(() => {
      return database.ref(`canvassing_calls/${roomId}/answer`).set({
        sdp: remoteWebRTCPeerConnection.localDescription.sdp,
        type: remoteWebRTCPeerConnection.localDescription.type,
      });
    })
...
window.rejectIncomingCall = function (roomId) {
  if (window._ringtoneDialogClose) window._ringtoneDialogClose();
  window.currentIncomingOffer = null;
  window.currentIncomingRoomId = null;
  ...
  database
    .ref(`canvassing_calls/${roomId}/status`)
    .set("rejected")
    .catch(console.warn);
  stopListeningForCallStatus();
...
window.endWebRTCCall = function (clearSignaling = true) {
  const roomIdBeingClosed = currentChatRoomId || window.currentIncomingRoomId;
  const wasActiveCall = !!(remoteWebRTCPeerConnection || localMediaStream);
  ...
  if (remoteWebRTCPeerConnection) {
    remoteWebRTCPeerConnection.close();
    remoteWebRTCPeerConnection = null;
  }

  iceCandidateListeners.forEach(
    ({ ref, callback }) =>
      ref.off("value", callback) || ref.off("child_added", callback),
  );
  ...
  if (clearSignaling && roomIdBeingClosed) {
    if (wasActiveCall) {
      database
        .ref(`canvassing_calls/${roomIdBeingClosed}/status`)
        .set("ended")
        .catch(console.warn);
    }
    setTimeout(
      () => {
        database.ref(`canvassing_calls/${roomIdBeingClosed}`).remove();
      },
      wasActiveCall ? 500 : 0,
    );
  }
