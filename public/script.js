...
function muatDataLengkap(callbackSelesai) {
  ...
  if (loggedInUser && loggedInUser.role !== "admin") {
    ...
    // LIMIT DIHAPUS: Mengambil seluruh data agar grafik admin membaca total seluruh nomor kontrak
    dbQuery = dbQuery.orderBy(firebase.firestore.FieldPath.documentId());
  }
...
function showLoginSuccessPopup(user) {
  ...
  let photoUrl = currentUserData.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=0D8ABC&color=fff`;
...
function initApp() {
  ...
  if (headerPhoto) {
    // [PERBAIKAN] Kembalikan penggunaan UI-Avatars sebagai fallback utama
    const avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=0D8ABC&color=fff`;
    headerPhoto.src = currentUserData.photo || avatarUrl;
  }
  ...
  setTimeout(() => {
    ...
    reader.onload = function (e) {
      try {
        ...
        // 2. Data Duplikat Diizinkan (Data dengan No. Kontrak sama akan ditumpuk/ditimpa)
        let newData = parsedData;

        // Jika data di dalam Excel kosong atau format tidak valid
        if (newData.length === 0) {
          hideLoading();
          return showDialog(
            "Upload Dibatalkan",
            "Tidak ada data valid di dalam file Excel yang diunggah.",
            "alert",
          );
        }
    ...
    // Tampilkan pop-up berhasil langsung (semua data masuk ke DB / ditumpuk)
            showSuccessModal(
              newData.length,
              targetUser,
...
function pushCanvassingPosition() {
  ...
  // [PERBAIKAN] Tambahkan encodeURIComponent untuk mencegah spasi memutus URL
  const userPhoto =
...
function mulaiCanvassing() {
  ...
  if (loggedInUser && loggedInUser.username) {
    ...
    // [PERBAIKAN] Tambahkan encodeURIComponent
    const userPhoto =
      currentUserFullData?.photo ||
      `https://ui-avatars.com/api/?name=${encodeURIComponent(userName)}&background=0D8ABC&color=fff`;
  ...
  // ... (sisa fungsi mulaiCanvassing() seperti navigasi Geolocation biarkan sama)
  ...
  database.ref("ocm_canvassing").on("value", (snapshot) => {
    ...
    if (data) {
      Object.keys(data).forEach((id) => {
       ...
       const displayAvatarsUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.nama || id)}&background=0D8ABC&color=fff`;
...
function notifyIncomingChat(fromUsername, roomId) {
  ...
  ringtoneAudio.loop = true; // nada dering diputar berulang
...
// Kirim pesan dgn tombol Enter, terpasang sekali via event delegation supaya
// tetap berfungsi berapa kali pun #chat-input dibuat ulang, dan tidak
// bergantung pada onclick tombol kirim di HTML (yang di screenshot bug report
// tampak memakai ikon yang sama dengan tombol "Lapor Bug" - kemungkinan salah
// pasang/salah fungsi).
...
function createPeerConnection(roomId) {
  ...
  pc.onconnectionstatechange = () => {
    if (pc.connectionState === "connected") setCallStatus("Tersambung");
    else if (
...
// Memulai panggilan (sisi penelepon)
window.startWebRTCCall = function (type) {
  ...
  if (remoteWebRTCPeerConnection) return; // sudah dalam panggilan
...
if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
  if (typeof showToast === "function") {
    showToast(
      "Akses kamera/mikrofon diblokir. Pastikan web menggunakan koneksi HTTPS yang aman!",
      "error",
    );
  }
  endWebRTCCall(true);
  ...
  navigator.mediaDevices
    ...
    .then(() => {
      database.ref(`canvassing_calls/${roomId}/offer`).set({
      ...
      // Dengarkan jawaban dari lawan bicara
    ...
    .catch((err) => {
      ...
      if (typeof showToast === "function")
        showToast("Tidak bisa mengakses kamera/mikrofon.", "error");
      endWebRTCCall(true);
    });
};

// Mendengarkan tawaran panggilan masuk dari lawan bicara (sisi penerima)
function listenIncomingCall(roomId) {
  ...
  chatCallListenerRef.on("value", (snapshot) => {
    ...
    const label = offer.callType === "video" ? "Video Call" : "Telepon";
    ...
    // Tampilkan dialog dengan dua tombol (Terima / Tolak)
    if (typeof showDialog === "function") {
      // Kita perlu custom dialog agar bisa menangani penolakan
      // Karena showDialog hanya mendukung 'confirm' (Ya/Batal), kita ubah menjadi custom
      // Atau gunakan fungsi baru: showIncomingCallDialog
      showIncomingCallDialog(roomId, offer, label);
    } else {
      acceptIncomingCall(roomId, offer);
    }
  });
}

function acceptIncomingCall(roomId, offer) {
  ...
  navigator.mediaDevices
    ...
    .then(() => {
      database.ref(`canvassing_calls/${roomId}/answer`).set({
        sdp: remoteWebRTCPeerConnection.localDescription.sdp,
        type: remoteWebRTCPeerConnection.localDescription.type,
      });
    })
...
// Mengakhiri panggilan. clearSignaling=false dipakai saat hentikanCanvassing()
// karena node canvassing_calls/{roomId} akan langsung dihapus total setelahnya.
window.endWebRTCCall = function (clearSignaling = true) {
  ...
  setCallStatus("Menunggu koneksi...");

  if (clearSignaling && currentChatRoomId) {
    database.ref(`canvassing_calls/${currentChatRoomId}`).remove();
  }
  currentCallType = null;
};

function showIncomingCallDialog(roomId, offer, label) {
  // Hentikan nada dering saat dialog ditutup
  const closeRingtone = () => {
    ringtoneAudio.pause();
    ringtoneAudio.currentTime = 0;
  };

  // Buat elemen dialog custom (bisa menggunakan modal yang sudah ada)
  const overlay = document.getElementById("custom-dialog-overlay");
  const box = document.getElementById("custom-dialog-box");
  if (!overlay || !box) {
    // Fallback: gunakan confirm biasa
    if (confirm(`${currentChatPeerId} mengirim ${label}. Terima panggilan?`)) {
      closeRingtone();
      acceptIncomingCall(roomId, offer);
    } else {
      closeRingtone();
      // Tolak panggilan (opsional, kirim sinyal penolakan)
      rejectCall(roomId);
    }
    return;
  }
  ...
  // Set konten dialog
  document.getElementById("custom-dialog-title").innerText =
    "📞 Panggilan Masuk";
  document.getElementById("custom-dialog-message").innerHTML = `
    <p class="text-sm"><strong>${currentChatPeerId}</strong> mengirim <strong>${label}</strong>.</p>
    <p class="text-xs text-slate-500 mt-1">Apakah Anda ingin menerima panggilan ini?</p>
  `;

  const btnContainer = document.getElementById("custom-dialog-buttons");
  btnContainer.innerHTML = `
    <button onclick="rejectIncomingCall('${roomId}')" class="px-5 py-2.5 text-sm font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-xl transition-all cursor-pointer">Tolak</button>
    <button onclick="acceptIncomingCall('${roomId}', ${JSON.stringify(offer).replace(/"/g, "'")})" class="px-5 py-2.5 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-all cursor-pointer">Terima</button>
  `;
  ...
  // Simpan referensi untuk memberhentikan ringtone saat dialog ditutup
  window._ringtoneDialogClose = closeRingtone;
}

// Tambahkan fungsi untuk menolak panggilan
window.rejectIncomingCall = function (roomId) {
  // Hentikan ringtone
  if (window._ringtoneDialogClose) window._ringtoneDialogClose();
  // Tutup dialog
  closeDialog();
  // Opsional: kirim sinyal penolakan ke lawan bicara (misal dengan menghapus offer)
  database.ref(`canvassing_calls/${roomId}/offer`).remove();
  if (typeof showToast === "function") showToast("Panggilan ditolak.", "info"); // 2. Filter Duplikat (memeriksa database saat ini)
